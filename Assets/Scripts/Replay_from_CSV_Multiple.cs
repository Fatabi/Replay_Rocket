...
public class Replay_from_CSV_Multiple : MonoBehaviour
{
    ...
    private List<float[]> time_s_List;
    private List<Vector3[]> phiThetaPsiDeg_List;
    private List<Vector3[]> xyz_m_List;
    ...
    void ChangeAircraftState()
    {
        int ct = 0;
        foreach (var aircraft in aircrafts)
            {
                xyz_m = xyz_m_List[ct];
                phiThetaPsiDeg = phiThetaPsiDeg_List[ct];
                time_s = time_s_List[ct];
    ...
    public void UpdateCSVList(){
        ...
        time_s_List = new List<float[]>();
        xyz_m_List = new List<Vector3[]>();
        phiThetaPsiDeg_List = new List<Vector3[]>();
        float maxTime = 0;
        for (int j = 0; j < inputFields.Length; j++)
        {
            selectedInputField = inputFields[j]; 
        
                if (File.Exists(selectedInputField.text))
                {
                    List<string> csvData = new List<string>();

                    using (StreamReader reader = new StreamReader(selectedInputField.text))
                    {
                    ...
                    time_s_List.Add(time_s);
                    phiThetaPsiDeg_List.Add(phiThetaPsiDeg);
                    xyz_m_List.Add(xyz_m);
