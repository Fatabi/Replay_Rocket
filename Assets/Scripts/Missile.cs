...
public class Missile : MonoBehaviour
{
        ...
        public void GenerateDataFromCSV(){
            if (File.Exists(inputField.text))
            {
                ...
                isLoad = true;
                FindInitialVelocity(xyz_m,time_s,out initVelocity, out startTime);
            }
            else
            {
    ...
    public void FindInitialVelocity(Vector3[] xyz_m,float[] time_s,out float initVelocity,out float startIdx){
            initVelocity = 0f;
            startIdx = 0;
            for (int i = 0; i < xyz_m.Length-1; i++)
            {
                float distance = Vector3.Distance(xyz_m[i],xyz_m[i+1]);
                float deltaTime = time_s[i+1] - time_s[i];
                if(distance > 0.001f){
                    initVelocity = distance/deltaTime;
                    startTime = time_s[i];
                    break;
                }
            }
